...
const ProfileCard = ({ teacher }) => {
  return (
    <div className="bg-white/80 backdrop-blur-md shadow-lg overflow-hidden rounded-2xl flex flex-col    space-y-4">
      ...
      <button className="ml-auto mr-6 lg:mr-1 lg:mt-10 border-2 border-blue-600 text-blue-800 px-2 py-1 rounded-xl hover:scale-105 transition-transform">
       Edit Profile
      </button>

      <div className='pb-1 pl-7 mt-4'>
        <p className='text-2xl text-blue-800'>{teacher.name}</p>
        <p className='ml-3 text-sm'>{teacher.desc}</p>
      </div>
